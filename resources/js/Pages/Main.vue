...
<template>
        ...
        <div class="main_wrapper">
            <div class="table_wrapper">
                <table>
                    ...
                    <tr v-for="product in props.products.data" v-on:click="current_product = product"
                        class="product_row">
                        <td>
                            <p>
                                {{ product.article }}
                ...
                <ul v-if="products.total > 0" class="paginate_list">
                    <li v-for="link in products.links">
                        <a
                            :href="link.url"
                            :class="[ 'paginate_link', {disabled: (link.url === null), active: parseInt(link.label) === products.current_page}]">
                       <span>
                           {{
            ...
            <div class="add_button_wrapper">
                ...
                <div class="filter_list">
                    ...
                    <ul class="sort_list">
                        <li class="sort_item">
                            <Link href="/products" :only="['products']"
                                  :class="['sort_link', { active: !$page.url.includes('available')}]">Все
                            </Link>
                        </li>
                        <li class="sort_item">
                            <Link href="/products/available" :only="['products']"
                                  :class="['sort_link', { active: $page.url.startsWith('/products/available')}]">Только
                                доступные
                            </Link>
                        </li>
                    </ul>
    ...
    <div v-if="current_product && !is_editing">
        <modal-show :product="current_product" @delete="update_current_product" @edit="is_editing = true"
                    @close="current_product = null"/>
    </div>
